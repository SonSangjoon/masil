...
interface RendererOptions {
  ...
  readonly debugControls?: boolean;
...
/** Continuously animated renderer for MASIL's responsive living presence. */
export function createRenderer(options: RendererOptions): Renderer {
  ...
  const setVitality = (value: number) => {
    vitalityTarget = Math.min(1, Math.max(0, value));
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      vitalityCurrent = vitalityTarget;
      requestMaterialDraw();
      return;
  ...
  const initialize = async () => {
    ...
    const guiModulePromise = options.debugControls
      ? import("lil-gui")
      : undefined;
