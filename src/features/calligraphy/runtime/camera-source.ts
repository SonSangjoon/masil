...
export type PendingCameraRequest = {
  readonly id: string;
  readonly source: Promise<CameraSource>;
  abort: () => void;
};

/**
 * Starts getUserMedia immediately so callers can invoke it inside the exact
 * click that confirms an air-writing choice. The canvas can mount later and
 * consume the same pending source without asking the person to click again.
 */
export function createPendingCameraRequest(): PendingCameraRequest {
  const controller = new AbortController();
  const source = requestCamera(controller.signal);
  // A renderer attaches its own handler after React mounts the canvas. Keep a
  // rejection observed during that short hand-off window.
