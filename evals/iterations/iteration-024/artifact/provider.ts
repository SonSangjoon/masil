  ...
  if (
    ...
    const personFacingGuidance =
      action === "preview"
        ...
        : action === "move"
          ? "Confirm in natural language that move.spokenMove was legal and visibly completed. Speak nextTurnOwner from the conversation viewpoint: agent means 'my turn' ('제가 둘 차례예요') and person means 'your turn' ('어르신 차례예요'), never a bare side phrase such as 'Han's turn'. If actor is agent, use first person and describe any preceding person move separately. Never merge actors. Avoid tool terms and unused coordinates."
          : undefined;
    return {
      ...result,
      structuredContent: {
      ...
      const webMcpTool: BrowserWebMcpTool = {
        ...
        execute: async (input) => {
          try {
            ...
            if (
              descriptor.name === "masil_open_activity" &&
              input.activity === "janggi"
            ) {
              const liveState = projectMasilWebMcpResult(
                "masil_get_janggi_state",
                await execute("masil_get_janggi_state", {}, "webmcp"),
              );
              result = {
                content: [
                  {
                    type: "text",
                    text: "Opened Janggi and returned the fresh live position, turn, coordinates, and legal destinations in the same WebMCP result.",
                  },
                ],
                structuredContent: {
                  ...(result.structuredContent ?? {}),
                  ...(liveState.structuredContent ?? {}),
                  stateIncludedWithOpen: true,
                  immediateStateReadNeeded: false,
                },
              };
            }
            const recovery =
