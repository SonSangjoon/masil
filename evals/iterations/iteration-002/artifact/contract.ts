...
export const MASIL_WEBMCP_CONTRACT_VERSION = "1.1.0";
...
export const MASIL_TOOL_DESCRIPTORS: readonly MasilToolDescriptor[] = [
  ...
  tool(
    "masil_get_janggi_state",
    "PRECONDITION: the Janggi scene is already visible. Never use this as the first call to start a game; call masil_open_activity first even if the person asks to inspect the board. Then read the exact position, turn, coordinates, and legal destinations. The provider is the rules authority.",
    emptyObjectSchema,
    true,
  ),
  tool(
    "masil_wait_for_person_janggi_move",
    "Use only when the person said they will make the Cho move directly on the board. Wait up to 45 seconds for that one gesture and resolve after validation and animation. Play one Han reply only when the person's current request explicitly asked the Agent to answer immediately. Never use this to wait for future speech.",
    emptyObjectSchema,
  ),
  tool(
    "masil_move_janggi_piece",
    "After opening Janggi and reading state, submit exactly one action to the rules authority. For preview, include the piece and one legal destination, use actor=person and personConfirmed=false, then stop without moving. For a requested move, submit the person's exact destination with actor=person and personConfirmed=true—even when it appears illegal—so the provider can reject it without substitution; explain a rejection and wait for correction. After a successful person move, do not play Han unless the person's current request explicitly asked the Agent to reply. Use actor=agent and personConfirmed=false only for that explicit reply. Success resolves after visible animation.",
    {
      type: "object",
      properties: {
        action: {
          ...
          description:
            "preview shows one legal destination without changing turn; move commits one exact move; pass/reset are consequential and require the person's explicit request.",
        },
        actor: {
          type: "string",
          enum: ["person", "agent"],
