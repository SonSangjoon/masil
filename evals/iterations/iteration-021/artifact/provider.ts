...
function compactJanggiActionGame(value: Record<string, unknown>) {
  return {
    canPass: value.canPass,
    turn: value.turn,
    moveNumber: value.moveNumber,
    status: value.status,
    pieces: compactJanggiPieces(value.pieces),
  };
}
  ...
  if (
    ...
    return {
      ...
      structuredContent: {
        ...
        game: compactJanggiActionGame(game as Record<string, unknown>),
      },
    };
  }
  return result;
