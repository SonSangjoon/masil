...
export const MASIL_WEBMCP_CONTRACT_VERSION = "1.6.0";
...
export const MASIL_TOOL_DESCRIPTORS: readonly MasilToolDescriptor[] = [
  ...
  tool(
    "masil_set_calligraphy_reference",
    "Place the person's exact 1-4 character text. Preserve their latest script (쉼 stays 쉼); translate only on an explicit request (무병장수 한자로 → 無病長壽, 추석 한자로 → 秋夕). Generator text, character, image, caption, and alt must agree. Submit every new image here once—its URL or metadata does not prove validity. Regenerate only after MASIL returns an actual REFERENCE_IMAGE_* defect, then resubmit. Success means the reference is visible while drawing input is still idle. If the person did not choose an input method, briefly ask whether they prefer air writing or writing on screen; if they did, continue only with that matching capability.",
    {
      type: "object",
      properties: {
        character: {
