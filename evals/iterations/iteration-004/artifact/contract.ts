...
export const MASIL_WEBMCP_CONTRACT_VERSION = "1.3.0";
...
export const MASIL_TOOL_DESCRIPTORS: readonly MasilToolDescriptor[] = [
  ...
  tool(
    "masil_set_calligraphy_reference",
    "Place the person's resolved exact 1-4 character text. Preserve the latest chosen script and wording exactly: 쉼 means character=쉼. Convert only when explicitly requested: '무병장수를 한자로' means character=無病長壽, and '추석 한자로' means character=秋夕. The generator text, character, image contents, caption, and alt text must all describe that same resolved text. Call immediately after opening calligraphy when the request already names it. First generate a 1536x1024 (3:2) PNG/WebP with real transparent alpha, black ink only, large complete horizontal text, at least 8% margins, and no seal or extra text. Fix any REFERENCE_IMAGE_* error before claiming success. After success: start camera only if explicitly requested; if the person explicitly refuses camera and wants touch/pointer writing, call masil_stop_calligraphy_camera; otherwise leave input choice to the person.",
    {
      type: "object",
      properties: {
        character: {
  ...
  tool(
    "masil_get_janggi_state",
    "PRECONDITION: the Janggi scene is already visible. Never use this as the first call; open Janggi first. Read the exact position, turn, coordinates, and legal destinations before grounding a spoken move. Reading is not a visible preview: when the person asks where a piece can go or asks to see a possible move, immediately follow this read with masil_move_janggi_piece action=preview for one useful legal destination, then stop for their choice. If the returned lastMove belongs to Cho and turnOwner=agent, acknowledge that person's move before making or describing the explicitly requested Han reply. The provider is the rules authority.",
    emptyObjectSchema,
    true,
  ),
  tool(
    "masil_wait_for_person_janggi_move",
    "Use when the person said they will make the Cho move directly on the board. Call after opening Janggi, wait up to 45 seconds for exactly that gesture, and resolve only after validation and visible animation. The result contains the person's completed move. If their current request explicitly asks the Agent to answer immediately, acknowledge that move, play exactly one legal Han move, then tell them both what they moved and what you answered. Otherwise stop after acknowledging their move. Never use this to wait for future speech.",
    emptyObjectSchema,
  ),
  tool(
    "masil_move_janggi_piece",
    "After opening Janggi and reading state, submit exactly one action to the rules authority. A spoken list of coordinates is not a preview. When the person asks to see where a piece can go, call action=preview with that piece and one useful legal destination, actor=person, personConfirmed=false; its success visibly highlights the route without moving, and you must stop for the person's choice. For a requested move, submit the person's exact destination with actor=person and personConfirmed=true—even when it appears illegal—so the provider can reject it without substitution; explain a rejection and wait for correction. After a successful person move, do not play Han unless the current request explicitly asked for an immediate reply. For that reply use actor=agent and personConfirmed=false exactly once. Your final person-facing sentence must acknowledge their completed move before briefly naming your reply; never imply the reply was their move. Success resolves after visible animation.",
    {
      type: "object",
      properties: {
        action: {
