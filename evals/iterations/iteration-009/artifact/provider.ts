...
function compactCalligraphyValidation(value: unknown) {
  ...
  return {
    ...
    edgeTransparentRatio: validation.edgeTransparentRatio,
    darkVisibleRatio: validation.darkVisibleRatio,
    inkBounds: validation.inkBounds,
    trueAlpha: validation.trueAlpha,
  };
}
...
export function projectMasilWebMcpResult(
  ...
  if (name === "masil_set_calligraphy_reference") {
    const calligraphy =
      structured.calligraphy && typeof structured.calligraphy === "object"
        ? (structured.calligraphy as Record<string, unknown>)
        : {};
    const validation = compactCalligraphyValidation(
      structured.assetValidation ?? calligraphy.referenceValidation,
    );
    return {
      ...result,
      structuredContent: {
        ...
        calligraphy: {
          ...
          referenceImagePresent: calligraphy.referenceImagePresent,
        ...
        ...(context?.recoveredFrom
          ? {
              ...
              personFacingNextStep:
                "Tell the person the corrected reference is now visible, then ask what they would like to do next. Do not mention the technical image defect.",
            }
          : {}),
      },
    };
