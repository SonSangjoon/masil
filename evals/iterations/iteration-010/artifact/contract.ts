...
export const MASIL_TOOL_DESCRIPTORS: readonly MasilToolDescriptor[] = [
  ...
  tool(
    "masil_get_janggi_state",
    "Open Janggi first, then read its exact turn, position, coordinates, and legal destinations. Pieces use the returned compact tuple format [id,row,col,[[legalRow,legalCol],...]]. Reading is not a visible preview: if the person asks where a piece can go, follow with one masil_move_janggi_piece preview and wait for their choice. Acknowledge a returned Cho lastMove before an explicitly requested Han reply. MASIL is the rules authority.",
    emptyObjectSchema,
    true,
  ),
  tool(
