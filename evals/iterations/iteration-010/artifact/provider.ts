...
function compactJanggiPieces(value: unknown) {
  ...
  return value.map((entry) => {
    ...
    const legalMoves = Array.isArray(piece.legalMoves)
      ? piece.legalMoves.map((move) => {
          if (!move || typeof move !== "object") return move;
          const destination = move as Record<string, unknown>;
          return [destination.row, destination.col];
        })
      : piece.legalMoves;
    return [piece.id, piece.row, piece.col, legalMoves];
  });
}

/** Losslessly encode the complete position without repeating object keys. */
export function projectMasilWebMcpResult(
  ...
  if (name === "masil_get_janggi_state") {
    return {
      ...
      structuredContent: {
        ...structured,
        pieceTupleFormat: "[id,row,col,[[legalRow,legalCol],...]]",
