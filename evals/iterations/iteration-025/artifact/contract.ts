...
export const MASIL_WEBMCP_CONTRACT_VERSION = "4.0.0";
...
export const MASIL_TOOL_DESCRIPTORS: readonly MasilToolDescriptor[] = [
  ...
  tool(
    "masil_get_janggi_state",
    "Open Janggi first, then read its exact turn, position, coordinates, and legal destinations. Reading is not a visible preview: if the person asks where a piece can go, show one masil_move_janggi_piece preview, ask for their choice, and end the Agent turn. A later spoken choice is a new turn and must use action=move; never call the board-gesture wait tool for future speech. Acknowledge a returned Cho lastMove before an explicitly requested Han reply. MASIL is the rules authority.",
    emptyObjectSchema,
    true,
  ),
  tool(
    "masil_wait_for_person_janggi_move",
    "Wait within the current turn for one direct Cho board touch/drag only when the person explicitly said they will move on the board now. Never use this tool after asking for a spoken choice; future speech starts a new Agent turn. The result is the person's validated visible move. Reply with one Han move only when explicitly requested, then acknowledge both moves.",
    emptyObjectSchema,
  ),
  tool(
    "masil_move_janggi_piece",
    "After reading state, submit one rules-authoritative action. To show where a piece can go, call preview once with one legal destination, actor=person, personConfirmed=false, then ask for the person's choice and end the turn; do not call the board-gesture wait tool. On their later spoken choice, call move with their exact destination, actor=person, personConfirmed=true. Submit an apparently illegal request unchanged so MASIL can reject it. Never play Han unless explicitly requested; then use actor=agent once and acknowledge the person's move first. Move success resolves after visible animation.",
    {
      type: "object",
      properties: {
        action: {
