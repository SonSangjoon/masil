...
export const MASIL_TOOL_DESCRIPTORS: readonly MasilToolDescriptor[] = [
  ...
  tool(
    "masil_stop_calligraphy_camera",
    "Stop pending or active camera input after the person's explicit request; preserve the reference and strokes, then enable direct touch/pointer writing. If camera input never entered requesting or hand mode, do not call this tool or claim direct drawing is ready.",
    {
      type: "object",
      properties: { personExplicitlyAsked: { type: "boolean" } },
      required: ["personExplicitlyAsked"],
